...
ModFunction void Initialize(AmethystContext* ctx) {
    ...
    ctx->mHookManager->RegisterFunction<&multiplayerGameinfoFromSessionProperties>("48 89 5C 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 8D AC 24 ? ? ? ? 48 81 EC ? ? ? ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 85 ? ? ? ? 48 8B F2 48 8B F9 48 89 4C 24 ? C7 44 24");
    ctx->mHookManager->CreateHook<&multiplayerGameinfoFromSessionProperties>(_multiplayerGameinfoFromSessionProperties, &multiplayerGameinfoFromSessionProperties);

    // sub_141025F30 (some member of Social::MultiplayerGameInfo)
    ctx->mHookManager->RegisterFunction<&multiplayerGameinfoToSessionProperties>("48 89 5C 24 ? 48 89 74 24 ? 48 89 7C 24 ? 55 41 56 41 57 48 8D AC 24 ? ? ? ? 48 81 EC ? ? ? ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 85 ? ? ? ? 48 8B F1 33 DB");
    ctx->mHookManager->CreateHook<&multiplayerGameinfoToSessionProperties>(_multiplayerGameinfoToSessionProperties, &multiplayerGameinfoToSessionProperties);
    ...
    ctx->mPatchManager->ApplyPatch(SlideAddress(0x9213AE), (uint32_t)NEW_PROTOCOL_VERSION); // Realms::GenericRequestHandler::send() patch

    //ctx->mPatchManager->ApplyPatch(SlideAddress(0x122DC00), (uint32_t)NEW_PROTOCOL_VERSION); // X-NetworkProtocolVersion HTTP header
    //ctx->mPatchManager->ApplyPatch(SlideAddress(0x122EB03), (uint32_t)NEW_PROTOCOL_VERSION); // X-NetworkProtocolVersion HTTP header

    ctx->mPatchManager->ApplyPatch(SlideAddress(0xC992CB), (uint32_t)NEW_PROTOCOL_VERSION); // Settings page
}
...
void* multiplayerGameinfoFromSessionProperties(int64_t a1, int64_t a2) {
    ...
    if (protoVer == NEW_PROTOCOL_VERSION) {
        protoVer = OLD_PROTOCOL_VERSION;
